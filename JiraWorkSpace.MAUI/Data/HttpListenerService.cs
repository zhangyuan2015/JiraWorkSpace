...
{
    internal static class HttpListenerService
    {
        ...
        public static void Start()
        {
            ...
            //定义url及端口号，通常设置为配置文件
            httpobj.Prefixes.Add("http://127.0.0.1:2504/");
            //启动监听器
            httpobj.Start();
        ...
        private static string HandleRequest(HttpListenerRequest request, HttpListenerResponse response)
        {
            ...
            try
            {
                ...
                //获取得到数据data可以进行其他操作
                ConfigService<SetupModel> configService = new ConfigService<SetupModel>(nameof(SetupModel));
                SetupModel setupModel = configService.GetEntity();
                if (setupModel != null && !string.IsNullOrWhiteSpace(setupModel.JiraDomain))
                {
                    List<CookieModel> cookies = JsonSerializer.Deserialize<List<CookieModel>>(data);
